...
messages) already lives in the global `CLAUDE.md`. **This skill is the
...
Approve or recommend merging only when the change improves code health and the
...
Issue or task · branch and PR · files changed · commands run · test or smoke
result · what works now · risks or follow-up · recommended next issue.
